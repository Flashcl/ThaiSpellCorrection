...
def _is_thai_and_not_num(word: str) -> bool:
    for ch in word:
        if ch != "." and not isthaichar(ch) and not ch.isalpha:
            return False
...
def repl(splits):
    return [L + c + R[1:] for L, R in splits if R for c in thai_letters+alphabet]
def insert(splits):
    return [L + c + R for L, R in splits for c in thai_letters+alphabet]
def _edits1(word: str) -> Set[str]:
    ...
    deletes =  pool.apply_async(dele,[splits]).get()
    transposes =  pool.apply_async(tranp,[splits]).get()
    replaces =  pool.apply_async(repl,[splits]).get()
    inserts =  pool.apply_async(insert,[splits]).get()
    pool.close()
    pool.join()
